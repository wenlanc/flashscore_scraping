...
class ShoppingCart
{
    ...
    public function manageShoppingCart()
    {
        if ($_POST['action'] == 'add') {
            ...
            @$_SESSION['shopping_cart'] = array_unique($_SESSION['shopping_cart']);
        }
        ...
        if (!empty($_SESSION['shopping_cart'])) {
            ...
            $result["tax"] = $this->getTaxValue($country_name);
            $result['cart_total'] = $this->getTotalCart($country_name);
        }
    ...
    public function getTax($country_name){
        ...
        for($i=0;$i<count($eu_countries);$i++){
            if(strpos(strtolower($country_name), strtolower($eu_countries[$i])) !== FALSE)
                return 0.2;
        }
        return 0;
    }
    
    public function getTaxValue($country_name){
        $cartItemSum = $this->getCartItems()["finalSum"];
        $tax = $cartItemSum * $this->getTax($country_name);
        return number_format($tax, 2);
    }
    public function getTotalCart($country_name){
        $cartItemSum = $this->getCartItems()["finalSum"];
        return number_format($cartItemSum + $this->getTaxValue($country_name), 2);
    }
}
